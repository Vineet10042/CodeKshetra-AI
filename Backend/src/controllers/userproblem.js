  ...
  try {
    for (const { language, completeCode } of referenceSolution) {
      ...
      for (const test of testResult) {
        if (test.status_id != 3) {
          const detail = test.compile_output ? `Compile error: ${test.compile_output}` : `Expected: ${test.expected_output || 'N/A'}. Got: ${test.stdout || test.stderr || 'N/A'}`;
          return res
            .status(400)
            .json({
              message: `Reference code failed! Status: ${test.status?.description || 'Error'}. Input: ${test.stdin || 'N/A'}. ${detail}`
            });
        }
      }
    }
...
const submittedProblem = async (req, res) => {
  try {
    ...
    if (ans.length == 0) return res.status(200).send("No Submission is persent");
